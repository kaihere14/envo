...
- `./.envo-config` in the current working directory — JSON `{"tag": ..., "owners": {tag: npub}}`, managed only by `helper::project_config`. `tag` is the default for `push`/`pull` run without one (like `git pull` after `git remote add`), resolved in `main.rs` via `resolve_tag()` and saved by the command with `remember_tag()` only after a push or pull succeeds, so a typo never becomes the default. `owners` is the per-tag owner pin. It is a trust anchor, so `write_config()` first adds it to the cwd's `.gitignore` (only inside a git repo, idempotent) and refuses to write if that fails, then writes through `write_secret()`.
